...
Public Class frmShedular
    ...
    'Dim RsTask As RadScheduler = New RadScheduler()
    ...
    Private Sub frmShedular_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        AddHandler RsTask.ActiveViewChanged, AddressOf RsTask_ActiveViewChanged
        AddHandler RsTask.ViewNavigated, AddressOf RsTask_ViewNavigated
    ...
    Private Sub RsTask_AppointmentElementDoubleClick(sender As Object, e As EventArgs) Handles RsTask.AppointmentElementDoubleClick
    ...
    Private Sub RsTask_AppointmentEditDialogShowing(sender As Object, e As AppointmentEditDialogShowingEventArgs) Handles RsTask.AppointmentEditDialogShowing, RSwo.AppointmentEditDialogShowing, RSEmp.AppointmentEditDialogShowing, RSEqp.AppointmentEditDialogShowing
        e.Cancel = True
    End Sub

    Private Sub RsTask_ContextMenuShowing(sender As Object, e As SchedulerContextMenuShowingEventArgs) Handles RsTask.ContextMenuShowing, RSwo.ContextMenuShowing, RSEmp.ContextMenuShowing, RSEqp.ContextMenuShowing
        If TypeOf e.Element Is AppointmentElement Then
            e.ContextMenu.Items.RemoveAt(0)
        End If
    End Sub

    Private Sub RsTask_AppointmentSelected(sender As Object, e As SchedulerAppointmentEventArgs)
    ...
    Private Sub addTaskAppointments()
        ...
        For i As Integer = 0 To summaries.Length - 1
            appointment = New Appointment(start(i), [end](i), summaries(i), "", "W/O : Wo1")
        ...
        For i As Integer = 21 To 30
            appointment = New Appointment(baseDate.AddDays(1).AddHours(6), baseDate.AddDays(1).AddHours(16), "Tsk " & (i + 1).ToString(), "", "W/O : Wo2")
            appointment.BackgroundId = AppointmentBackground.MustAttend
            Me.RsTask.Appointments.Add(appointment)
        Next i
    End Sub
    Private Sub addWoAppointments(ByRef sche As RadScheduler)
        ...
        Dim appointment As Appointment = Nothing
        For i As Integer = 0 To summaries.Length - 1
            appointment = New Appointment(baseDate.AddHours(i + 1), baseDate.AddHours(i + 6), summaries(i), "", "")
            appointment.BackgroundId = AppointmentBackground.Birthday
    ...
    Private Sub RsTask_AppointmentFormatting(sender As Object, e As SchedulerAppointmentEventArgs)
    ...
    Private Sub RsTask_ActiveViewChanged(sender As Object, e As SchedulerViewChangedEventArgs)
    ...
    Private Sub RsTask_ViewNavigated(sender As Object, e As SchedulerViewNavigatedEventArgs)
    ...
    Private Sub RSwo_ScreenTipNeeded(sender As Object, e As ScreenTipNeededEventArgs) Handles RSwo.ScreenTipNeeded
        ShowToolTip(TryCast(e.Item, AppointmentElement))
    End Sub

    Private Sub RsTask_ScreenTipNeeded(sender As Object, e As ScreenTipNeededEventArgs) Handles RsTask.ScreenTipNeeded
        ShowToolTip(TryCast(e.Item, AppointmentElement))
    End Sub

    Private Sub RSEqp_ScreenTipNeeded(sender As Object, e As ScreenTipNeededEventArgs) Handles RSEqp.ScreenTipNeeded
        ShowToolTip(TryCast(e.Item, AppointmentElement))
    End Sub

    Private Sub RSEmp_ScreenTipNeeded(sender As Object, e As ScreenTipNeededEventArgs) Handles RSEmp.ScreenTipNeeded
        ShowToolTip(TryCast(e.Item, AppointmentElement))
    End Sub

    Private Sub ShowToolTip(Item As AppointmentElement)
        Dim appointmentElement As AppointmentElement = Item
        If appointmentElement IsNot Nothing Then
            Dim screenTip As New RadOffice2007ScreenTipElement()
            screenTip.CaptionLabel.Text = appointmentElement.Appointment.Summary.ToString()
            screenTip.MainTextLabel.Text = appointmentElement.Appointment.Location.ToString()
            appointmentElement.ScreenTip = screenTip
        End If
    End Sub

    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Dim frmprint As frmPrint = New frmPrint(Me)
        frmprint.StartPosition = FormStartPosition.CenterParent
        frmprint.ShowDialog(Me)
    End Sub

    Private Sub RSwo_ContextMenuShowing(sender As Object, e As SchedulerContextMenuShowingEventArgs) Handles RSwo.ContextMenuShowing
